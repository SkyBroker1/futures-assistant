...
def collect_options_vola():
    ...
    try:
        if ENABLE_DVOL: raise RuntimeError("deribit_dvol_placeholder")
...
def collect_macro_flows():
    ...
    try:
        ...
        if total<=0.0: raise RuntimeError("defillama_zero")
...
def build_tripack_meta(index_items):
    ...
    try:
        ...
        st_total=mf.get("stables",{}).get("total"); etf_rows=mf.get("etf",{}).get("rows",0); etf_source=mf.get("etf",{}).get("source")
        if etf_source=="sosovalue": meta["conf"]-=0.05; meta["log"]["flags"].append("etf:fallback:sosovalue")
        ...
        _log_err(f"meta:macro {e}"); meta["log"]["quorum"]="ok
