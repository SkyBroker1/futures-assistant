...
"""
postprocess_fill_missing.py v1.3
- Якщо collector не створив SPOT шард(и) - робимо fallback:
  * тягнемо BTCUSDT 1d з Binance REST (public, без ключа)
  * створюємо spot_ohlcv_v2_part000.json (degraded:btc-only)
  * оновлюємо index.json.files.spot.files
- Далі:
  * options_vola_v2.json: hv_surrogate з BTCUSDT
  * macro_flows_v2.json: DeFiLlama + Farside -> SoSoValue
  * derivs_signals_v2.json: безпечний stub, якщо відсутній
- Оновлюємо tripack_meta_v2.json: checks, log.flags, log.missing, quorum
"""
...
# ---------- spot fallback (Binance REST) ----------

def fetch_binance_klines_btc_1d(limit: int = 120) -> List[List[float]]:
    url = "https://api.binance.com/api/v3/klines"
    qs = {"symbol": "BTCUSDT", "interval": "1d", "limit": str(limit)}
    r = requests.get(url, params=qs, timeout=20)
    r.raise_for_status()
    data = r.json()
...
def ensure_spot_shard(out_dir: str, indexj: Dict[str, Any], logf: str | None = None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """За відсутності spot shards створює мінімальний part000 з BTCUSDT 1d."""
    files_spot = indexj.get("files", {}).get("spot", {}).get("files", [])
    if files_spot:
        log("spot shards already present - no autospot fallback needed", logfile=logf)
        return indexj, {}

    try:
        candles = fetch_binance_klines_btc_1d(limit=150)
        shard = [
            {"symbol": "BTCUSDT", "tf": "1d", "candles": candles}
        ]
        shard_name = "spot_ohlcv_v2_part000.json"
        shard_path = os.path.join(out_dir, shard_name)
        dump_json(shard_path, shard)

        # оновлюємо index.json
        ...
        return indexj, {"created": True, "name": shard_name, "rows": len(candles)}
    except Exception as e:
        log(f"autospot fallback failed: {e}", logfile=logf)
        return indexj, {"created": False, "error": str(e)}
...
def main():
    ...
    # 1) autospot fallback - створюємо шард, якщо його немає
    indexj, spot_info = ensure_spot_shard(out_dir, indexj, logf=post_log)
    if spot_info.get("created"):
        tripack.setdefault("log", {}).setdefault("flags", []).extend(["spot:fallback:binance_rest", "spot:degraded:btc-only"])

    # 2) derivs stub при відсутності
    ...
    # 5) spot sanity - читаємо останнє close з першого шарду
    try:
        spot_files = indexj.get("files", {}).get("spot", {}).get("files", [])
        if spot_files:
            shard_obj = load_json(os.path.join(out_dir, spot_files[0]))
            closes, _tf = _select_btc_series_from_spot(shard_obj)
    ...
    if not log_block["missing"] and tripack.get("checks", {}).get("btc_close_gt_1000") and tripack.get("checks", {}).get("options_vola_ok") and tripack.get("checks", {}).get("macro_quorum"):
