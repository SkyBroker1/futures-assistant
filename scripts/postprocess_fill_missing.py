...
"""
postprocess_fill_missing.py v1.7
Стратегія фолбеків з «твердим» seed, щоб кворум завжди збирався.

Порядок:
SPOT: existing -> Binance 1d -> CoinGecko 1d -> last price (без spot-файлу)
VOLA: hv з свічок -> hv_const=60 (seed-прапор)
MACRO: DeFiLlama+Farside -> SoSoValue -> cache(static/cache) -> seed(static/seed)
DERIVS: stub, якщо collector не дав

Усі причини у tripack_meta_v2.json.log.flags. Якщо використали seed - quorum=ok_seed.
"""
...
def fetch_binance_klines_btc_1d(limit: int = 180) -> List[List[float]]:
    ...
    if len(out) < 30: raise RuntimeError(f"too few candles from Binance: {len(out)}")
    return out
...
def fetch_coingecko_btc_daily(limit_days: int = 180) -> List[List[float]]:
    ...
    if len(arr) < 30: raise RuntimeError(f"too few points from CoinGecko: {len(arr)}")
...
def hv_from_closes(closes: List[float], tf: str = "1d") -> float:
    if len(closes) < 30: raise RuntimeError("not enough closes for hv")
    rets = []
    for i in range(1, len(closes)):
        p, c = closes[i-1], closes[i]
        if p>0 and c>0: rets.append(math.log(c/p))
    if len(rets) < 20: raise RuntimeError("not enough returns for hv")
...
def fetch_etf_farside() -> List[Dict[str, Any]]:
    ...
    for tr in table.find_all("tr")[1:]:
        ...
        if len(tds)>=3: rows.append({"date": tds[0], "issuer": tds[1], "flow": tds[2]})
...
def main():
    ...
    # base files
    if not ensure_file(index_path): dump_json(index_path, {"files":{"spot":{"files":[]}}, "conf":0.0}); log("index.json created minimal", logfile=post_log)
    if not ensure_file(tripack_path): dump_json(tripack_path, {"log":{"quorum":"fail","missing":[],"flags":[]}, "checks":{}, "conf":0.0}); log("tripack_meta_v2.json created minimal", logfile=post_log)
    if not ensure_file(run_meta_path): dump_json(run_meta_path, {"run_id": args.gh_run_id}); log("run_meta.json created minimal", logfile=post_log)
    ...
    if not ensure_file(macro_path):
        ...
        etf_rows, etf_source = [], "farside"; flags=[]
        try:
            ...
            try:
                ...
                if cache and cache.get("rows"): etf_rows = cache["rows"]; etf_source="cache"; flags.append("macro:fallback:cache")
        if st_total is None:
        ...
        if not ok_macro:
            ...
            if seed:
                ...
                st_total = (seed.get("stables") or {}).get("total")
                etf_rows = (seed.get("etf") or {}).get("rows") or []
                etf_source = (seed.get("etf") or {}).get("source") or "seed"
                ok_macro = True
